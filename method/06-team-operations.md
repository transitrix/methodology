...
- `raised_by` — the adopter's own role that raised the finding. Any role the adopter's model admits is valid. The four shipped guide names (`Ingest`, `Modeler`, `Analyst`, `Validator`) are compatible assistant-activity labels, not enterprise permission grants; an adopter with a richer role model uses their own role names. Preserve historical values. For new access assignments, separate expertise, scope, operation rights, and change responsibility as described in the [enterprise reference](../guides/repository-mcp-enterprise.md).
