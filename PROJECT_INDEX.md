# LiteEA Project Index & Navigation

Complete guide to the LiteEA project structure and where to find everything.
...
| **TOOLING.md** | Tools & integrations guide | Setting up LiteEA BAT or other tooling |
...
├── 0.archive/                   # Archive for old/deprecated elements
...
```bash
mkdir -p organizations/my_company/{elements/{01_motivation,02_business,03_application,04_technology},relations,.templates,.validators,views,0.archive}
...
2. Then: `method/LiteEA Методология...` (full methodology)
...
1. Core: `method/LiteEA Методология...`
...
- PROC-ORD-FULFILL.bpmn.yaml           # Order fulfillment process
...
### 0.archive/ - Historical Records

Where to move deprecated/old elements instead of deleting them.

```
Example structure:
0.archive/
├── LEGACY_SERVICE_v1.yaml
├── OLD_ROLE.yaml
└── DEPRECATED_PROCESS.bpmn.yaml
```
...
| Task | See File |
|------|----------|
| Understand the methodology | `method/LiteEA Методология...` |
...
| Learn BPMN format | `organizations/[org]/.templates/bpmn/process_template.bpmn.yaml` |
...
→ Read `method/LiteEA Методология...`
