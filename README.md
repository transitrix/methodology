...
- **[`patterns/`](patterns/)** — Concrete deployment patterns for common scenarios (Transitrix Alone, Knowledge Store, Network Catalogue, ADR Registry, MCP Server exposure, Enterprise Memory, Personal Memory, Baseline & Audit Trail).
...
- **[Desktop model workplace](guides/desktop-model-workplace.md)** — Qualified read-only setup paths for ChatGPT Desktop and Claude Code Desktop, with connection and evidence checks.
- **[Repository MCP for enterprises](guides/repository-mcp-enterprise.md)** — Recommended scoped access, compatible assistant modes, deployment choices, and acceptance scenarios.
...
- **[Adopter guides](guides/README.md)** — recommendations, proposals, and task and component procedures that don't fit the reading order above (unnumbered — a menu, not a sequence): modelling capabilities, modelling complex processes, adopting the Architecture Decision Log, how a release records the systems it was assembled on, how a model-backed document prints, how a model-backed document is versioned, [how to preserve issued document versions](guides/how-to-preserve-issued-document-versions.md).
...
2. Linters enforce syntax, atomicity, referential integrity, and policy on every commit. Type and layer rules are available via `@transitrix/cli --scope=repo`.
