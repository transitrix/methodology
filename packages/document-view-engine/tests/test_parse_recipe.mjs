#!/usr/bin/env node
// Unit tests for src/parse-recipe.mjs — every syntax form in the document-view
// engine epic's §2, plus header validation (§1) and the defined error cases.
//
// Run: node packages/document-view-engine/tests/test_parse_recipe.mjs
...
function recipe(body, headerExtra = '') {
...
{
  const { header, errors } = parseRecipe(recipe(''));
...
{
  const { errors } = parseRecipe(recipe('', 'profile: srs-legacy\n'));
...
{
  const { ast, errors } = parseRecipe(recipe('{{ REQ-14 }}'));
...
{
  const { ast, errors } = parseRecipe(recipe('{{ REQ-14.text }}'));
...
{
  const { ast, errors } = parseRecipe(recipe('{{ REQ-14.parent.title }}'));
...
{
  const { ast, errors } = parseRecipe(recipe('literal \\{{ not a directive }}'));
...

{
  ...
  const { ast, errors } = parseRecipe(recipe(body));
...

{
  ...
  const { ast, errors } = parseRecipe(recipe('{{# each REQUIREMENT }}{{ .id }}{{/ each }}'));
...
{
  const { ast, errors } = parseRecipe(recipe('{{ trace from = REQUIREMENT to = VERIFICATION via = verifies }}'));
...
{
  const { errors } = parseRecipe(recipe('{{ trace from = REQUIREMENT to = VERIFICATION }}'));
...
{
  const { ast, errors } = parseRecipe(recipe('{{ view path/to/view-file }}'));
...
{
  const { ast, errors } = parseRecipe(recipe('{{ view path/to/view-file as = fig-arch fit = page }}'));
...
{
  const { errors } = parseRecipe(recipe('{{ view path/to/view-file fit = tall }}'));
  check(errors.some((e) => /fit must be one of width\/page\/none/.test(e.message)), 'invalid fit value is flagged');
}

{
  const { errors } = parseRecipe(recipe('{{ view }}'));
...
{
  const { ast, errors } = parseRecipe(recipe('{{ figure path/to/image.png caption = "Device, front" as = fig-device }}'));
...
{
  const { ast, errors } = parseRecipe(recipe('{{ figure path/to/image.png }}'));
...
{
  const { ast, errors } = parseRecipe(recipe('{{ figref fig-arch }}'));
...
{
  const { errors } = parseRecipe(recipe('{{ figref }}'));
  check(errors.some((e) => /requires a name/.test(e.message)), 'figref with no name is flagged');
}

{
  const { errors } = parseRecipe(recipe('{{ figref fig-arch fig-other }}'));
...
{
  const { ast, errors } = parseRecipe(recipe('Before {{ REQ-14 }} after.'));
