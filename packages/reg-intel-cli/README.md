...
| `digest [org-root] [--run-id <id>] [--as-of YYYY-MM-DD] [--out <path>] [--scope <word>]` | ✅ | Assemble the human review digest (`review-digest.yaml`) — staged SEGMENT / candidate / AMENDMENT artefacts grouped by codex source (candidates via `derived_from` → SEGMENT), with each source's scan block + a tally. `gate.admits_to_canon: false`. Non-destructive default: an unresolved `review-digest.yaml` already at the flat path is never overwritten — a concurrent run lands under its own dated `review-digest-<scope>-YYYYMMDD-<seq>/` directory (`--scope` defaults to `batch`; see `@transitrix/ingest-cli`'s README "Multi-batch naming", same mechanism — vkgeorgia/strategy#837). Schema: [`schemas/review-digest.schema.json`](../../transitrix/skills/reg-intel/schemas/review-digest.schema.json). |
