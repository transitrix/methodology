...
export async function defaultDigestPath(orgRoot, { scope, content, runId } = {}) {
  const dir = join(resolve(orgRoot), '_intake', 'processing');
  if (!(await exists(dir))) await mkdir(dir, { recursive: true });
  return resolveBatchPath({ processingDir: dir, filename: 'review-digest.yaml', scope, content, runId });
}
