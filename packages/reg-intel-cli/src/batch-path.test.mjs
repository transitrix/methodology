...
// Run: node --test packages/reg-intel-cli/src/batch-path.test.mjs
...
test('resolveBatchPath: no existing file returns flat path', async () => {
  ...
  try {
    ...
    assert.equal(result, join(testDir, 'digest.yaml'));
  } finally {
    await cleanup();
  }
});
...
test('resolveBatchPath: existing file with different run_id returns dated path (different batch)', async () => {
  ...
  try {
    ...
    assert.ok(result.includes('digest-batch-'), `path should include dated batch: ${result}`);
    assert.ok(result.endsWith('digest.yaml'));
  } finally {
    await cleanup();
  }
});
...
test('resolveBatchPath: multiple dated directories increment sequence number', async () => {
  await setup();
  try {
    const dir1 = join(testDir, 'digest-batch-20260827-1');
    const dir2 = join(testDir, 'digest-batch-20260827-2');
    ...
    assert.ok(result.includes('digest-batch-20260827-3'), `should use next sequence number: ${result}`);
  } finally {
    await cleanup();
  }
});
...
test('resolveBatchPath: malformed run_id line is not extracted', async () => {
  ...
  try {
    ...
    assert.ok(result.includes('digest-batch-'));
  } finally {
    await cleanup();
  }
});
