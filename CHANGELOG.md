...
- **`documents` package — identity and traceability for issued documents** (`notations/packages/documents.md`, `notations/PACKAGES.md` §7.1). Optional, removable, experimental (reviewed by 2027-03-02). Object types `document-type` (`doct-*`) and `document` (`doc-*`); one-way `canon_refs` onto core elements; no core envelope. A repository that omits `packages: [documents]` is unchanged. The spec adds a DMS integration contract (the seam an external document-management system binds to) and three on-demand traceability queries derived from run records and git history, with no persistent index: `documents_citing_element`, `stale_documents_for_change`, `document_provenance`. New validation rules `DOCS-001..007`. Worked example: `notations/examples/packages/documents/`. (#567, #574, #580)
- **`guides/how-the-model-binds-to-an-issue-tracker.md` — how an adopter joins the model to the tracker their delivery teams already use.** States the property that makes the binding portable: the work hierarchy stays in the model, where `ACTION` is recursive across `Initiative` / `Programme` / `Project` / `Task` via `parent`, so the tracker is asked to carry at most one level of it and never the shape. Two pointers, one in each direction — `ACTION.link` from the element to the work item, and an `Action: ACTION-…` tag from the work item back to the element. For the tag the guide gives two options with their costs stated: a line in the issue description, which needs no administrator and is identical in every tracker, or a dedicated custom field, which buys reliable querying at the price of an administrator, an instance-wide namespace, and portability. Both carry the same value under the same name, so moving between them is a change of location rather than of meaning. Includes where to stop modelling — `Project` for most adoptions, `Task` where traceability requires it — and notes that a tracker's own hierarchy above its epic level is often a paid tier, which an adoption following this guide does not need. Linked from `method/06-team-operations.md` §"Not a ticket tracker", which previously said to keep your existing tracker without saying how to connect it. (#565)
- **`guides/how-the-model-binds-to-an-issue-tracker.md` §When the ticket carries a requirement — the same binding serves requirements traceability.** Teams write requirements and acceptance criteria into tickets because that is where the work is; the guide now covers that case with the same two pointers and a different element type (`Requirement: REQUIREMENT-…`, one tag per element referenced). States the rule the case turns on: the obligation's text lives in the catalogued `REQUIREMENT` and never in the ticket, because a ticket is closed, archived and eventually migrated out, while an obligation outlives all three — a requirement whose only full statement sits in a closed ticket is an archive being searched, not traceability. Places acceptance criteria as `VERIFICATION` against the requirement and a satisfaction claim as `ASSERTION`, so the chain reads end to end — need → requirement → what realises it → what verified it → which release it was required for — from either end and without the tracker surviving. (#566)
- **Knowledge-object supersession — `supersedes` / `superseded_by`** (`patterns/knowledge-store.md`). Optional fields on a knowledge object's front matter. When understanding changes, a curator creates a new object and points both ways rather than rewriting the original in place. Validation: `KS-018` (error, unresolved target), `KS-019` (warning, pair incomplete), `KS-020` (error, pointer identity mismatch). Templates updated. A repository that never sets the pair validates as before. (#569)
- **Knowledge-object freshness warnings.** `FRESHNESS-001` now also flags a knowledge object whose `timestamp` is older than `confidence_decay.knowledge.stale_days` (defaults 730 days; configured in `transitrix.yaml`, `CONTRACT.md` §11.3 / `MANIFEST.md`). Advisory only — never blocks, never mutates, never filters. Canon-element freshness is unchanged. (#576)
- **Knowledge-store compaction design frame** (`patterns/knowledge-store.md`). Records the invariants a future compaction algorithm must satisfy and four visible variants; the algorithm itself is not shipped. Worked example of the supersession pattern added. (#577, #578)
- **Document-renderer print surface — paged media, landscape, and figure captions.** `@transitrix/document-renderer` generates paged-media CSS for running footers, lays a wide view on a landscape page, and formats figure captions with snapshot dates. Engine choice for HTML-to-PDF (Vivliostyle) and the paged-media contract are documented in `packages/document-renderer/PDF_RENDERING_ENGINE.md`. (#579, #581, #582, #583)
- **README Contributing — where work is filed.** Work for this repository is filed in the family headquarters, not this repository's issue tracker; external contributors open a pull request here or email. (#584)

### Changed

- **Feedback Record `raised_by` accepts any role the adopter's model admits**, not only the four shipped guides (`Ingest`, `Modeler`, `Analyst`, `Validator`). Those four remain the defaults and the examples; an adopter with a richer role model uses their own names. (`method/06-team-operations.md` §3.2, onboard `FINDINGS.md` template, feedback skill). (#563)
- **PlantUML rendering pinned to 1.2026.6** across the published surfaces that invoke it (`integration/plantuml.md` and the version-check workflow). (#562)

### Fixed

- **PlantUML rendering is a VS Code capability**, not a Studio one — `integration/plantuml.md` now says so. (#564)
- **Feedback-register version boundary** that still named a never-tagged `4.3.0` now names `5.0.0`, matching the release that actually shipped the directory layout. (#575)
