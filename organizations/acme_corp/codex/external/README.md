...
An external artefact's `jurisdiction:` frontmatter MUST match its parent folder (`CODEX-001`). One `<ID>.yaml` per artefact (`LAW-…`, `REGULATION-…`). See `notations/14-codex.md` for the frontmatter contract.
