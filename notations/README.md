...
| [24-rules-in-force.md](./views/reports/24-rules-in-force.md) | `rules-in-force` | Report-config view over the `codex` zone — every admitted `LAW` / `REGULATION` / `STANDARD` / `POLICY` / `INTERNAL_STANDARD` artefact currently in force, with the `REQUIREMENT`s drawn from each via `derived_from`; `PRINCIPLE` is out of scope by definition (§1). | `*.rules-in-force.transitrix.yaml` | draft |
