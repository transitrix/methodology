...
| [30-sdd.md](./views/documents/30-sdd.md) | `sdd` | Software Design Description — groups admitted `APPLICATION` / `NODE` / `TECHNOLOGY_SERVICE` design elements under themselves, listing the `REQUIREMENT`s each realises via the `ASSERTION` trace. | `*.sdd.transitrix.yaml` | draft |
