...
**Scope:** Where saved, named view-configs live in an adopter repository, how they're named, how a reader lists them, and how a reader (or a tool, or the future report skill) re-runs one by name. This document **extends and codifies the existing report-config shape** declared by the report-config view specs ([`11-scenarios.md`](11-scenarios.md), [`21-compliance-impact.md`](21-compliance-impact.md), [`22-coverage-metric.md`](22-coverage-metric.md)) — it does not introduce a new notation. Companion to the *reports rendered from declarative view-configs* architecture decision, Decision §1, §5, §7 (Step 1).
...
A named view-config is **the parameter artefact of a report** (per the *reports rendered from declarative view-configs* architecture decision, Decision §1):
...
- discoverable by name, so a reader (or the report skill — see the same decision, §7 Step 3) can list and select reports without inspecting file contents.
...
Re-running a saved view-config is a deterministic re-render — the renderer reads `(view-config, canon)` and emits the report (per the *reports rendered from declarative view-configs* architecture decision, §2). Two re-runs against the same canon produce identical output; two re-runs against a *changed* canon surface the canon change as a diff in the report, never as a configuration drift.
...
The CLI surface above is the convention this document fixes. The CLI implementation itself is tooling, delivered separately (per the same decision, §7 Step 1 / Step 3).

---

## 6. Zero-configuration default

Every report-producing view spec ([`11-scenarios.md`](11-scenarios.md) §4, [`21-compliance-impact.md`](21-compliance-impact.md) §4 + §4.1 and [`22-coverage-metric.md`](22-coverage-metric.md) §4 + §4.1) declares **explicit defaults** for every non-required field. A view-config that carries only the required envelope (`notation:`, `spec_version:`, `methodology_version:`, `view.id`, `view.name`) renders deterministically — each omitted field falls back to its spec default.
