...
| 1 | `type: Initiative` (alias `Strategic Initiative`) | Tree roots — direct children of the virtual root |
| 2 | `type: Programme` | Children of an Initiative |
| 3 | `type: Project` | Children of a Programme (or Initiative) |
| 4 | `type: Task` | Leaf nodes within a Project |

The `type` value labels the **semantic scale**; the `parent` field carries the structural edge. Both are optional on any element — an action without `type` is displayed at its inferred level (derived from the `parent` chain depth); an action without `parent` is treated as a root (level 1 if `type: Initiative`, otherwise an orphan — see §8).
...
view_config:
  ...
  display:
    ...
    show_type_badge: true            # show level badge (Initiative / Programme / etc.)
...
| `show_type_badge` | no | `true` | Show the `type` level badge per node. |
| `show_action_card_link` | no | `true` | For Project-level nodes, show a link to the Action Card document if one references this action (`type: Project`). |
...
- **Tree view:** rendered as a root node labelled with `virtual_root_label` (defaults to the organisation name from the adopter manifest). All Initiative-level actions (`type: Initiative`, or unclassified actions with no `parent`) are direct children.
