...
### 3.7 Verification type — moved to the `design-controls` package

Through methodology 2.1.0 this section registered `VERIFICATION` — the engineering V&V counterpart to `ASSERTION` (§3.6). As of 2026-07-29 `VERIFICATION`, along with `HAZARD` / `RISK_CONTROL` (formerly §3.1) and `DC_TRACE_MATRIX` (formerly §3.2), moved to the `design-controls` domain package and no longer appear in this closed core registry. Per [`PACKAGES.md`](PACKAGES.md) §4.1, a package's own TYPEs are disjoint from this registry by construction; see [`packages/design-controls.md`](packages/design-controls.md) §2 for the package's own registry of these TYPEs, and ADR `methodology/2026-07-29-design-controls-as-a-package` / epic `vkgeorgia/strategy#852` for the decision.
