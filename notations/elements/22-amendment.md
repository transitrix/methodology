...
| `source` | yes | string | Typed canonical ID of the codex artefact that was amended. MUST resolve to an admitted codex artefact whose TYPE is one of `LAW`, `REGULATION`, `STANDARD`, `POLICY`, `INTERNAL_STANDARD` (`AMENDMENT-002`). |
...
| `AMENDMENT-002` | error | `source` is missing, malformed, does not resolve to any admitted codex artefact, or resolves to an artefact whose TYPE is not one of `LAW`, `REGULATION`, `STANDARD`, `POLICY`, `INTERNAL_STANDARD`. An AMENDMENT records a change to a codex source; nothing else can be amended in this sense. |
