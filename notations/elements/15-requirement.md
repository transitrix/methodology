...
The shared lifecycle (`LIFECYCLE-001..004`, [CONTRACT.md](../CONTRACT.md) §7.3) and header (`HDR-001..004`, [CONTRACT.md](../CONTRACT.md) §2) rules apply to REQUIREMENT files in addition to the REQ-* rules above. The aggregated compliance-and-design-controls-domain rules table (covering REQUIREMENT, ASSERTION, VERIFICATION, HAZARD, and RISK_CONTROL) lives in [CONTRACT.md](../CONTRACT.md) §8.
...
- §4 — `REQ-VERIF-COVERAGE-001` / `REQ-VERIF-COVERAGE-002`, the reverse-trace completeness rules for the REQUIREMENT ↔ `VERIFICATION` leg ([27-verification.md](27-verification.md)): no verification targets the requirement at all, or every one that does is still unresolved (`not_yet_run` / `inconclusive`). Resolves the "deliberately out of scope" question left open when `VERIFICATION` landed. Paired with the risk-control-side reverse-trace rules, `HAZ-RISKCTL-COVERAGE-001`/`-002` and `RISKCTL-VERIF-COVERAGE-001`, in [28-hazard-risk-control.md](28-hazard-risk-control.md) §6.
