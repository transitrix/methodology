...
The shared lifecycle (`LIFECYCLE-001..004`, [CONTRACT.md](../CONTRACT.md) §7.3) and header (`HDR-001..004`, [CONTRACT.md](../CONTRACT.md) §2) rules apply to REQUIREMENT files in addition to the REQ-* rules above. The aggregated compliance-domain rules table (covering REQUIREMENT, ASSERTION, and their cross-cutting checks) lives in [CONTRACT.md](../CONTRACT.md) §8.

**Design-controls package.** A REQUIREMENT's engineering V&V coverage — whether a `VERIFICATION` targets it, and whether that verification closed — was `REQ-VERIF-COVERAGE-001`/`-002` here through v0.6. As of 2026-07-29 those rules, and the `VERIFICATION` TYPE they check, moved to the `design-controls` domain package ([`../packages/design-controls.md`](../packages/design-controls.md), [`../packages/design-controls/verification.md`](../packages/design-controls/verification.md) §5); see §5 below. A repository that does not declare `packages: [design-controls]` has no such coverage question to answer — `REQ-COVERAGE-001` (compliance coverage, above) is unaffected and remains the only cross-cutting coverage check core runs against REQUIREMENT.

---

## 5. Evolution

**Removed (2026-07-29):** `REQ-VERIF-COVERAGE-001` / `REQ-VERIF-COVERAGE-002` (landed v0.6, below) moved to the `design-controls` domain package along with the `VERIFICATION` TYPE they check — see §4 and [`../packages/design-controls.md`](../packages/design-controls.md). Per ADR `methodology/2026-07-29-design-controls-as-a-package` and epic `vkgeorgia/strategy#852`. `REQ-COVERAGE-001` is unaffected.

**Landed (v0.6, 2026-07-26):**
- §4 — `REQ-VERIF-COVERAGE-001` / `REQ-VERIF-COVERAGE-002`, the reverse-trace completeness rules for the REQUIREMENT ↔ `VERIFICATION` leg (formerly `27-verification.md`, now [`../packages/design-controls/verification.md`](../packages/design-controls/verification.md)): no verification targets the requirement at all, or every one that does is still unresolved (`not_yet_run` / `inconclusive`). Resolved the "deliberately out of scope" question left open when `VERIFICATION` landed. Paired with the risk-control-side reverse-trace rules, `HAZ-RISKCTL-COVERAGE-001`/`-002` and `RISKCTL-VERIF-COVERAGE-001`, in the same package spec.
...
- The engineering V&V counterpart, `REQ-VERIF-COVERAGE-001`/`-002`, and its `VERIFICATION` TYPE — moved to the `design-controls` package: [`../packages/design-controls.md`](../packages/design-controls.md), [`../packages/design-controls/verification.md`](../packages/design-controls/verification.md).
