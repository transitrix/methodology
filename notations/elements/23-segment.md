...
| `source` | yes | string | Typed canonical ID of the codex artefact the chunk was extracted from. MUST resolve to an admitted codex artefact whose TYPE is one of `LAW`, `REGULATION`, `STANDARD`, `POLICY`, `INTERNAL_STANDARD` (`SEGMENT-002`). A SEGMENT is always a chunk of a codex source; nothing else is segmented in this sense. |
...
| `SEGMENT-002` | error | `source` is missing, malformed, does not resolve to any admitted codex artefact, or resolves to an artefact whose TYPE is not one of `LAW`, `REGULATION`, `STANDARD`, `POLICY`, `INTERNAL_STANDARD`. A SEGMENT is always a chunk of a codex source. |
