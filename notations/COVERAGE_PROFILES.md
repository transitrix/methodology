...
- **Codex artefacts.** `LAW` / `REGULATION` / `STANDARD` / `POLICY` / `INTERNAL_STANDARD` / `PRINCIPLE` live in the `codex/` zone, not `canon/elements/<layer>/`, and are governed by the `codex` notation ([`elements/14-codex.md`](elements/14-codex.md)) and the manifest's `zones:` field — not by this profile.
