...
**Design-controls package.** Through methodology 2.1.0 this table also aggregated three more TYPEs extending the same spine with engineering V&V + ISO 14971 risk — `VERIFICATION`, `HAZARD`, `RISK_CONTROL`. As of 2026-07-29 those TYPEs and their rules (`VERIF-*`, `HAZ-*`, `RISKCTL-*`, and the `HAZ-RISKCTL-COVERAGE-*` / `RISKCTL-VERIF-COVERAGE-001` cross-cutting checks) moved to the `design-controls` domain package — [`packages/design-controls.md`](packages/design-controls.md) §5 carries their own aggregated rules index now. Per ADR `methodology/2026-07-29-design-controls-as-a-package` and epic `vkgeorgia/strategy#852`. `REQ-VERIF-COVERAGE-001`/`-002` moved with them; only `REQ-COVERAGE-001` remains in the table below.
...
**Design-controls package rules** (`VERIF-*`, `HAZ-*`, `RISKCTL-*`, and the `HAZ-RISKCTL-COVERAGE-*` / `RISKCTL-VERIF-COVERAGE-001` cross-cutting reverse-trace checks) moved to [`packages/design-controls.md`](packages/design-controls.md) §5 — see the note above this table. They apply only in a repository that declares `packages: [design-controls]`; a repository that does not is unaffected by them.
