...
**Promotion trigger — when (and only when) referenced from another document.** Canon ratified `standalone`-with-promotion over an always-standalone alternative on the grounds that the flat DGCA / Goals / Action schedule documents are *narrative artefacts*, not catalogue layouts: fragmenting a DGCA document into per-element files from day one would harm the very authoring ergonomics those notations exist for. So an element stays inline in its authoring document until a *second* document references it; at that point it is promoted to its catalogue file and both documents reference it by ID. The supporting tool work — detecting a cross-document reference, enforcing promotion at that point, and flagging an inline reference that should already have been promoted — is a validator / codemod concern tracked separately (it does not change the file schema this document defines).
...
The split between `canon/elements/` (and canonical `canon/relations/`, `canon/assertions/`) and `views/` is governed by one invariant: **the elements and relations are the complete and sufficient source of truth for the organisation's behaviour; a view is a projection over them.** Formally, a view is `render(Elements + Relations, view_config)` — it takes the canonical elements and relations plus its own *presentation configuration* (which elements to show, grouping, filtering, ordering, display options) and renders them. The folders own disjoint things:

- **No behaviour lives only in a view.** Every fact about how the organisation works — a process's steps and flow, a goal hierarchy, a capability's maturity — must be reconstructable from `canon/elements/` and `canon/relations/`. Delete the entire `views/` folder entirely and no knowledge is lost; all views regenerate from elements and relations.
- **No view configuration lives in `canon/`.** Which rows a Process Blueprint shows, a capability-map's orientation, a saved report's selection and filters — these are the view's own primary data and are not derivable from elements and relations. They are not behaviour; they stay in the view document.

**View-purity corollary.** A view carries *no non-derivable information beyond its configuration* — no hand-tuned layout, no view-only annotations, no canonical content that exists nowhere else. (Layout is already computed deterministically per notation, so it carries no information — consistent with this rule.) BPMN is the case this resolves: a process's flow is behaviour and belongs in the canonical `PROCESS` element (§7.5); the BPMN view file is a projection of it, stored in `views/bpmn/`.
...
- `codex` artefacts (`LAW` / `REGULATION` / `STANDARD` / `POLICY` / `INTERNAL_STANDARD`) live in the `codex/` zone, not `canon/elements/`, and follow [`elements/14-codex.md`](elements/14-codex.md). They are not `canon` elements and are out of scope here.
...
- **`EQUIPMENT` and `BUSINESS_OBJECT` are `standalone`, promoted from `view-defined` (ADR 2026-06-08).** Both were originally Process Blueprint document-local TYPEs; the ADR promoted them to first-class catalogued elements — `EQUIPMENT` as the first `04_technology` TYPE (§7.14), `BUSINESS_OBJECT` (renamed from `INFORMATION_ENTITY` for ArchiMate alignment) at `02_business` (§7.15). `INFORMATION_ENTITY` is the **retired** former name of `BUSINESS_OBJECT` — its alias window closed at 1.0.0 (`BOBJ-D001`, error); it carries no row of its own in §4, matching how the still-open aliases (`FACTOR`, `ACTIVITY`) are omitted from this table.
...
- **Content vs presentation config.** The element TYPEs in §4 are content (an `INTEGRATION`, a `REGISTRY` row — real things the organisation has). A view document *also* carries presentation configuration — which elements it shows, grouping, filtering, ordering, display options. That configuration is the view's own primary data, is not an element TYPE, and legitimately and permanently lives in the view (out of scope of this element table by construction). The rule: content is promotable to canon; presentation config stays in the view.
- **`view-defined` content must be promotable.** `INTEGRATION` already is (§4.1) — nested-in-view today, with canonical-grammar IDs ready for promotion. `SCENARIO`'s reclassification to a standalone content element (§4.1, §7.18) settled its half of this tension — the scenarios view is now a report-config rendering surface over the `SCENARIO` catalogue, not a content home; `EQUIPMENT` and `BUSINESS_OBJECT` settled theirs by promotion to `standalone` (ADR 2026-06-08). No non-promotable `view-defined` rows remain — the former `ISSUE` row was retired (2026-06-07).
...
| `falsifier` | no | string | The observation that would weaken or refute this driver as a standing force. A property of the driver itself, not of any one `ASSESSMENT` of it — the force is what the falsifier bears on, so it lives here rather than on §7.17. Prose, not a structured condition: whether the falsifier has fired is a judgement a person makes when reviewing a new `ASSESSMENT` against it, not a stored boolean on this element. The record exists so a future view could list drivers whose falsifier condition appears to have fired; that view is not specified here. Omitted means no falsifier has been recorded, not that the driver admits none. |
...
**Retired former name.** `INFORMATION_ENTITY` was accepted as an alias for one release; that window **closed at 1.0.0** (CHANGELOG — "`INFORMATION_ENTITY-*` IDs and the `information_entities:` field in process-blueprint are hard errors"). It is no longer an accepted spelling: `BOBJ-D001` is an error, not a warning.

The name is still *listed* — in `vocabulary.yaml`'s `deprecated_element_types` with `accepted: false`, `retired_in: "1.0.0"` — for one reason only: so a tool that meets the old name can say what to rename it to instead of complaining about an unknown TYPE. Listing it is not acceptance. Migrate: rename `element_type: INFORMATION_ENTITY` → `BUSINESS_OBJECT`, rename ID prefix `INFORMATION_ENTITY-` → `BUSINESS_OBJECT-`, rename the blueprint array field `information_entities[]` → `business_objects[]`.
...
| `geographic_scope` | no | list of string | Where the finding was observed. The primitive is the **country**, reusing the codex jurisdiction vocabulary ([elements/14-codex.md](elements/14-codex.md) §1.1) rather than inventing a second one: each entry is an ISO 3166-1 alpha-2 code or `eu` (EU-wide). A **region** (a named grouping such as "APAC") is expressed as the explicit list of country codes it comprises — no named-region vocabulary is introduced; a finding whose geography can't be reduced to specific codes is not a codeable scope and does not belong in this field (`description` carries it instead). A **market** (a commercial/business segment, e.g. "the enterprise segment") is not modelled by this field at all — it is a property of the business dimension the finding bears on, not of geography, and belongs in the `DRIVER`'s own `category` / `description` (§7.1) if it needs recording. The reserved single-entry value `[global]` asserts the finding holds everywhere with no narrower scope; omitting the field instead means the geography was **not stated** — the two are different claims and are not interchangeable. `global` MUST NOT combine with any other entry in the same list (`ASSESS-001`). |
...
- **No driver / capability / process / application / product composition lives here.** Those were the v0.x view-defined scenario's content. Composition belongs to `TARGET_STATE` (the structural snapshot, §7.17); cross-layer impact analysis belongs to the DGCA chain and to the structural views. The reclassification splits those concerns out cleanly.
...
**Citing an external standard.** A term taken from an external standard (a regulator's defined-terms section, an industry glossary) cites the codex artefact it was drawn from via `derived_from` instead of restating the standard's own text as the definition. Where `derived_from` is present on a `TERM`, every entry MUST resolve to an admitted `LAW`, `REGULATION`, `STANDARD`, `POLICY`, `INTERNAL_STANDARD`, or `PRINCIPLE` codex artefact (`TERM-002`) — the same restriction [elements/15-requirement.md](elements/15-requirement.md) §4 applies to `REQUIREMENT.derived_from` (`REQ-003`), for the same reason: a definitional citation names a source of authority, not a piece of Field evidence.
...
| `BOBJ-D001` | error | An element carries `element_type: INFORMATION_ENTITY` or an `INFORMATION_ENTITY-…` id prefix — the retired former name of `BUSINESS_OBJECT`, whose alias window closed at 1.0.0. Rename `element_type` to `BUSINESS_OBJECT`, update the id prefix, and rename `information_entities[]` → `business_objects[]` in blueprints. At the candidate stage the finding is surfaced as a validation flag and never silently rewritten (candidates are drafts a human reviews); it is not admissible to canon. |
...
| `TERM-002` | error | A `TERM.derived_from` entry resolves to an admitted artefact whose TYPE is not one of `LAW`, `REGULATION`, `STANDARD`, `POLICY`, `INTERNAL_STANDARD`, `PRINCIPLE` (or does not resolve to any admitted codex artefact at all). A TERM derives only from codex source documents — the same restriction `REQ-003` applies to `REQUIREMENT.derived_from` ([elements/15-requirement.md](elements/15-requirement.md) §4). |
| `ASSESS-001` | error | An `ASSESSMENT.geographic_scope` entry is present but empty, or contains a value that is not `global`, an ISO 3166-1 alpha-2 code, or `eu`; or combines `global` with any other entry in the same list. Reuses the codex jurisdiction vocabulary ([elements/14-codex.md](elements/14-codex.md) §1.1, §7.17). |
