...
**Package:** part of the `design-controls` domain package — see [`../design-controls.md`](../design-controls.md) for the package's own spec (name/folder, removal procedure, experimental status). Not a core notation: nothing here is admitted, validated, or rendered by core tooling in a repository that does not declare `packages: [design-controls]` ([`../../PACKAGES.md`](../../PACKAGES.md) §5).

**Scope:** Two element TYPEs, `HAZARD` and `RISK_CONTROL`, that together carry the ISO 14971 risk-management chain — hazard → control → requirement → verification. The shared header / admission / lifecycle contracts are defined in [CONTRACT.md](../../CONTRACT.md); the package's own TYPE registry is [`../design-controls.md`](../design-controls.md) §2.

`HAZARD` and `RISK_CONTROL` each live as a single YAML file under `design-controls/hazards/` or `design-controls/risk-controls/` respectively, in a declaring adopter repository — a top-level package folder, sibling to `canon/` ([`../../PACKAGES.md`](../../PACKAGES.md) §3), not nested inside it. Each is named by its canonical ID, carrying the admission record ([CONTRACT.md](../../CONTRACT.md) §6, `zone: canon`) plus the primitive lifecycle ([CONTRACT.md](../../CONTRACT.md) §7) and the type-specific frontmatter below — the same admission-record/lifecycle shape core primitives carry, inherited unchanged from when these TYPEs shipped as core. Bundled in one spec because they are two halves of the same shape of work — a source of harm and the measure that addresses it — the way [15-requirement.md](../../elements/15-requirement.md) bundles `REQUIREMENT` and `CONSTRAINT`.

---

## 1. The chain

ISO 14971 (medical-device risk management) names a repeating chain: a **hazard** (a potential source of harm) is addressed by a **risk control** (a measure that reduces the risk it poses), which is typically realised as one or more **design requirements**, which are in turn checked by **verification**. Transitrix already has `REQUIREMENT` ([15-requirement.md](../../elements/15-requirement.md)) and, as of this release, `VERIFICATION` ([verification.md](verification.md)); `HAZARD` and `RISK_CONTROL` complete the chain:
...
`RISK_CONTROL.mitigates` is the **forward** link to the hazard(s) it addresses; `RISK_CONTROL.satisfies` is the **forward** link into the existing REQUIREMENT/VERIFICATION spine. Neither `HAZARD` nor `REQUIREMENT` carries a backward field — as with `REQUIREMENT` ← `ASSERTION.about` ([16-assertion.md](../../elements/16-assertion.md) §1) and `REQUIREMENT` ← `VERIFICATION.verifies` ([verification.md](verification.md) §1), the reverse direction is answered by scanning the referencing catalogue, not by an inline field on the referenced element. Whether every hazard has an adequate control, and every control an adequate verification, is a cross-cutting completeness question answered by `HAZ-RISKCTL-COVERAGE-001` / `-002` and `RISKCTL-VERIF-COVERAGE-001` (§6).
...
```
design-controls/hazards/<ID>.yaml
design-controls/risk-controls/<ID>.yaml
```

One artefact per file, named by its canonical ID, in the package's top-level folder ([`../design-controls.md`](../design-controls.md) §1) — not under `canon/`. Examples:

- `design-controls/hazards/HAZARD-BATTERY-DEPLETION-1.yaml`
- `design-controls/risk-controls/RISK_CONTROL-LOW-BATTERY-ALERT-1.yaml`

The two folders sit alongside each other under `design-controls/`, peers, not nested. A generic worked example exercising both TYPEs alongside `REQUIREMENT` and `VERIFICATION` lives at [`../../examples/packages/design-controls/`](../../examples/packages/design-controls/).
...
| `RISKCTL-VERIF-COVERAGE-001` | warning | A `RISK_CONTROL` carries `satisfies`, but the `REQUIREMENT` it references has no admitted `VERIFICATION` that has reached `outcome: pass` or `outcome: fail` (either none targets it at all, or every one that does is still `not_yet_run` / `inconclusive`). The same underlying gap as `REQ-VERIF-COVERAGE-001` / `-002` ([15-requirement.md](../../elements/15-requirement.md) §4), surfaced directly on the RISK_CONTROL record so an ISO 14971 audit reader does not have to jump to the requirement file to see that a risk-mitigating design requirement lacks V&V closure. Does not fire when `satisfies` is absent — an early-lifecycle control recorded before decomposition into a formal design requirement (§3) is not an orphan. |
...
**Reverse-trace completeness.** Whether every `HAZARD` has at least one adequately-controlled `RISK_CONTROL` (`HAZ-RISKCTL-COVERAGE-001` / `-002` above), and whether every `RISK_CONTROL` that carries `satisfies` has a corresponding, closed `VERIFICATION` (`RISKCTL-VERIF-COVERAGE-001` above) — these cross-cutting questions require scanning the full hazard / risk-control / requirement / verification catalogues together. Paired with the REQUIREMENT-side reverse-trace rules in [verification.md](verification.md) §5 and [15-requirement.md](../../elements/15-requirement.md) §4.

---

## 7. Evolution

**Landed (v0.3, 2026-07-29):** moved from core (`notations/elements/28-hazard-risk-control.md`) into the `design-controls` domain package, per ADR `methodology/2026-07-29-design-controls-as-a-package` (`vkgeorgia/strategy` architecture hub) and epic `vkgeorgia/strategy#852`. No schema field changed; the two TYPEs, their grammar, and their admission-record/lifecycle shape are unchanged from v0.2.
...
- **SDS / trace-matrix rendering.** The design-controls trace matrix including the risk chain (Hazard → Risk-Control → Requirement → Verification) now renders as a report-config view: [`24-design-controls-trace-matrix.md`](trace-matrix-view.md).
...
- The REQUIREMENT element type a RISK_CONTROL may satisfy: [15-requirement.md](../../elements/15-requirement.md).
- The VERIFICATION type that closes the chain: [verification.md](verification.md).
