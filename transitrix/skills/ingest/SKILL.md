...
The same `source_hash` deduplicates: if the identical content was already admitted to the zone, `admit-source` **skips** it (naming the existing artefact) rather than minting a second one — pass `--force` to admit a deliberate duplicate. (When two artefacts share a hash, `review-queue` surfaces the cluster under `duplicate_sources`.) This applies to both the `field` and `codex` routes, and it holds across a naive re-run: the first successful admit moves the raw source out of `_intake/inbox/` (field: into `_intake/processed/`; codex: renamed into the artefact's `sources/` folder), so a retry that does not re-drop the raw file still finds it there and is still recognised as the same source — it does not silently fall through to minting an unfingerprinted duplicate. Path handling here is `node:path`-based throughout, so this holds identically on a Windows workstation or a Linux CI runner.
