...
- `routing` — the role's recommendation; the human makes the final call regardless. `escalate-methodology` is written into `operations/feedback.md` as a Feedback Record (`method/06-team-operations.md` §3.2) — a different, fixed field set from this raise-time shape, suited to a standalone register entry rather than an in-flow finding.
