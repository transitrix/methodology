...
The named guides select assistant modes, not enterprise roles or access grants. Keep expertise, accessible scope, operation permissions, and per-change assignment separate. Validator supports an assigned domain expert; automated validation, expert review, and approval remain distinct. Preserve existing adopter guides and tool pointers; apply a reviewed in-place migration instead of overwriting custom policy. See the [compatible migration guide](https://github.com/transitrix/methodology/blob/main/guides/repository-mcp-enterprise.md#14-compatible-role-and-onboarding-migration).
...
- `templates/ANALYST.md` → `<repo-root>/ANALYST.md` — the **Analyst** mode guide. The Analyst is a read-only assistant that answers questions about the organisation from the validated `canon/` model. It is always scaffolded alongside `AGENTS.md`; the user activates it by pointing their assistant at `ANALYST.md` instead of `AGENTS.md` when asking a business question.
- `templates/analyst-mcp.json` → `<repo-root>/.mcp.json` — **optional local filesystem example**, enabled only after the adopter chooses trusted local retrieval and independently enforced restrictions in `ANALYST.md` §6. This server exposes writes as well as reads; it is not Repository MCP or enterprise authorization. Pin a tested package version and resolve the directory for the actual client. Preserve unrelated entries and any existing `canon` configuration; do not silently replace it. For a governed service, use its approved client configuration instead. Never install or expand retrieval access just because a role guide exists.
- `templates/VALIDATOR.md` → `<repo-root>/VALIDATOR.md` — the **Validator** mode guide. The assistant supports expert review by checking a change (a PR, a local diff) for structural validity, whole-repo referential integrity, and blast radius before it merges. It is always scaffolded alongside `AGENTS.md`; the user activates it by pointing their assistant at `VALIDATOR.md` instead of `AGENTS.md` when reviewing a change. Use repo-wide file tools only when authorized for that scope; otherwise report restricted coverage and request an authorized check. A successful formal check does not replace expert review or approval.
...
1. **Whole-repo validator** — fetch `https://raw.githubusercontent.com/transitrix/methodology/v<methodology_version>/tools/lint.py` and write it to `<repo-root>/.validators/lint.py`. This is the model-integrity linter: it scans `canon/elements/**` and `canon/relations/**` for atomicity, referential integrity, and policy. The canonical source is `tools/lint.py` at the methodology root — fetch the canon, **not** the acme-corp reference repo's copy.
...
- **External** (law / regulation / externally issued standard given to the org by outside authority) — copy `templates/codex-external.yaml` to `codex/external/<jurisdiction>/<ID>.yaml`. Rename the file to the artefact's canonical ID (e.g. `LAW-PERSONAL-DATA-2017-1.yaml`, `REGULATION-GDPR-2016-1.yaml`, `STANDARD-iso-27001-1.yaml`). The `<jurisdiction>` folder MUST match the `jurisdiction:` field inside the file (rule `CODEX-001`) — use ISO 3166-1 alpha-2 (`ge`, `de`, …), `eu` for EU-wide, or `intl` for internationally issued SDO artefacts (ISO, IEC, IEEE, and peers). For `STANDARD`, also set `issuing_authority:` to the issuing body.
- **Internal, POLICY or INTERNAL_STANDARD** (the org can state who enforces it and how conformance is checked) — copy `templates/codex-internal.yaml` to `codex/internal/<ID>.yaml`. Internal artefacts are not foldered by jurisdiction. Rename the file to the artefact's canonical ID (e.g. `POLICY-DATA-RETENTION-1.yaml`, `INTERNAL_STANDARD-coding-conventions-1.yaml`).
- **Internal, PRINCIPLE** (a rule the org holds over itself with no stated issuing authority or conformance test — `14-codex.md` §2.1) — copy `templates/codex-principle.yaml` to `codex/internal/<ID>.yaml`, same folder convention. Rename to the artefact's canonical ID (e.g. `PRINCIPLE-DATA-MINIMISATION-1.yaml`). Ask the user for `statement:` (one normative sentence) and `rationale:` — both required — and only ask for `issuing_authority:` if they say the artefact does have a named enforcer, in which case reclassify it as a POLICY instead and use `templates/codex-internal.yaml`.

Ask the user for the artefact's own content — `id:`, `name:`, `description:`, the codex-specific fields (`jurisdiction` + `effective_date` for external; `issuing_authority` required on `STANDARD` in addition; `issuing_authority` + `effective_date` for internal POLICY/INTERNAL_STANDARD; `statement` + `rationale` for internal PRINCIPLE), and `gate_checks.source_authority` (the issuing legislative body, SDO, internal authority, or whoever admitted the artefact — real content, not a default). Compute `admitted_at` / `admitted_by` yourself per "Admission record and lifecycle" above rather than asking the user to fill them in. A codex artefact stores the **source document only** — do not add bindings to canonical entities or processes on the artefact itself. Those bindings live downstream on `REQUIREMENT.derived_from` (which cites the codex source) and on `ASSERTION` (which links each requirement to its subject). See `notations/elements/14-codex.md` §8 Migration for the rationale; `notations/elements/15-requirement.md` and `notations/elements/16-assertion.md` for the downstream shapes.
...
| What binds the organisation today — every admitted law, regulation, policy, and internal standard, with the requirements drawn from each | **Rules in Force** | `*.rules-in-force.transitrix.yaml` |
...
|---|---|---|---|
| External law, regulation, or SDO-issued standard binding the org | **codex** | `codex/external/<jurisdiction>/<ID>.yaml` | `LAW` / `REGULATION` / `STANDARD` |
...
- **Process Blueprint** — `notation: process-blueprint`. Root key `process_blueprint:` with `stages[]` (each carrying `goal` and `result`) and per-aspect arrays `systems[]`, `actors[]`, `equipment[]`, `business_objects[]`. Aspect entries reference the stages they appear in via `stages: [STAGE-…]`.
...
- **Rules in Force** — `notation: rules-in-force`. Report-config over the codex zone itself (sibling of Compliance Impact and Coverage Metric — same `REQUIREMENT.derived_from` join key, but reads the source catalogue rather than the obligation × subject overlay). Root key `view:` declaring `scope.codex` (`include` or `filter` by jurisdiction / TYPE), `grouping`, `order_by`, and `summary`. Carries no canonical content — lists every admitted `LAW` / `REGULATION` / `STANDARD` / `POLICY` / `INTERNAL_STANDARD` artefact with its per-rule `REQUIREMENT` count (a zero count renders as a counted row, never an omission). `PRINCIPLE` is out of scope by definition — it names no issuing authority and no conformance test.
...
- **Codex** *(zone primitives, not a view document)* — each artefact is a single `<ID>.yaml` file under `codex/external/<jurisdiction>/` (external: `LAW` / `REGULATION` / `STANDARD`) or `codex/internal/` (internal: `POLICY` / `INTERNAL_STANDARD` / `PRINCIPLE`). No `notation:` header; carries the admission record (`CONTRACT.md` §6, `zone: codex`, `gate_checks.source_authority`) plus codex frontmatter (external: `jurisdiction` + `effective_date`; `STANDARD` also requires `issuing_authority`; internal POLICY/INTERNAL_STANDARD: `issuing_authority` + `effective_date`). A codex artefact stores the **source document only** — bindings to entities and processes live on `REQUIREMENT.derived_from` (`notations/elements/15-requirement.md`) and on `ASSERTION` (`notations/elements/16-assertion.md`), not on the codex artefact itself.
...
| MCP config — optional local filesystem example | `templates/analyst-mcp.json` | `<repo-root>/.mcp.json` *(only after access setup; preserve existing entries)* |
...
| Rules in Force | `templates/rules-in-force.rules-in-force.transitrix.yaml` |
...
| Internal (principles) | `templates/codex-principle.yaml` | `codex/internal/<ID>.yaml` |
