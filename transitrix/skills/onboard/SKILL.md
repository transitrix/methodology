...
3. **CI gate** — `WebFetch` `https://raw.githubusercontent.com/transitrix/methodology/v<methodology_version>/integration/ci-example.yaml` and write it to `<repo-root>/.github/workflows/architecture-validate.yaml`. It runs validation separated by responsibility — repo structure + manifest, model integrity (elements + relations) via `lint.py`, and view notations via `npx @transitrix/cli validate` — and blocks merges on failure.
